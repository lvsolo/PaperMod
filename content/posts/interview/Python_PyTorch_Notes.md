...
## 3.4 in-place 操作 vs autograd

对**需要梯度**的 tensor 做 in-place（`+=`、`copy_`、`index_add_`…）可能报：

> RuntimeError: a leaf Variable that requires grad is being used in an in-place operation

因为反向传播需要这个中间值，原地改了就回不去了。**解法**：用 `out=` 的非 in-place 版，或先 `.clone()`。`.data` 时代的老写法别用了（见 §3.8）。
