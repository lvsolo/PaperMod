...
class DINOLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        """
        student_out = (student_output / self.student_temp).chunk(self.ncrops)  # 切成 ncrops 份
        temp = self.teacher_temp_schedule[epoch]  # teacher 温度
        teacher_out = F.softmax((teacher_output - self.center) / temp, dim=-1).detach().chunk(2)  # detach! 切成 2 份(global)
        total = 0
        n = 0
        for iq, q in enumerate(teacher_out):  # q: 某个 global 视角
            for v in range(len(student_out)):  # v: 某个 student 视角
                ...
                n += 1
        total_loss = total / n
        self.update_center(teacher_output)  # 更新 centering
        return total_loss
