...
class VisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x):
        ...
        ⚠️ 提示(隐藏坑): multi-crop 时 global 和 local 分辨率不同 → patch 数不同,
        而 self.pos_embed 是按【global 的 patch 数】预建的, 直接相加会尺寸不匹配。
        需要: 当 patch 数对不上时, 把 pos_embed 的 patch 部分双三次插值到当前网格
        (参考 timm 的 VisionTransformer / _solution/model.py)。
        """
        x = self.patch_embed(x)
        if x.shape[1] != self.pos_embed.shape[1] - 1:
            interped_pos_embed = F.interpolate(
                    self.pos_embed[:,1:,:].transpose(2,1).reshape(
                        self.pos_embed.shape[0], self.embed_dim,
                        int(math.sqrt(self.pos_embed.shape[1]-1)),
                        int(math.sqrt(self.pos_embed.shape[1]-1))),
                    size=(int(math.sqrt(x.shape[1])), int(math.sqrt(x.shape[1]))), mode='bicubic', align_corners=False).flatten(2).transpose(2,1)
            interped_pos_embed = torch.cat([self.pos_embed[:, :1, :], interped_pos_embed], dim=1)
        else:
            interped_pos_embed = self.pos_embed
        x = torch.cat((self.cls_token.expand(x.size(0), -1, -1), x), dim=1)
        x = x + interped_pos_embed
        return x
