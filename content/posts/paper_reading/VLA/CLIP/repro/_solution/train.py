...
def retrieve_topk(query_emb, candidate_emb, k):
    ...
    """
    # ===== 已填(夜间自动): top-k 检索 =====
    sim = query_emb @ candidate_emb.t()                # (M, N) 余弦相似度
    return sim.topk(k, dim=1).indices                  # (M, k) 每个 query 最相似的 k 个 candidate
...
def retrieval_eval_flickr(model, test_ds, eval_tf, device, ks=(1, 5, 10), batch_size=64):
    ...
    for i in range(0, len(test_ds), batch_size):
        imgs = torch.stack([eval_tf(to_pil(test_ds[j]["image"]).convert("RGB"))
...
def main():
    ...
    model = CLIPWrapper(tokenizer.vocab_size, embed_dim=args.embed_dim, max_len=tokenizer.max_len).to(device)
