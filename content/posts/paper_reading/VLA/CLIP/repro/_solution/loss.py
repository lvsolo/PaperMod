...
class CLIPLoss(nn.Module):
    ...
    def clip_contrastive_loss(self, image_emb, text_emb, logit_scale):
        ...
        """
        # ===== 已填(夜间自动): 对称 InfoNCE =====
        logits = image_emb @ text_emb.t() * logit_scale.exp()       # (N, N) 相似度×温度, 对角线=正样本
        labels = torch.arange(logits.shape[0], device=logits.device)
        loss_i2t = F.cross_entropy(logits, labels)                  # 图→文: 第 i 张图匹配第 i 句文本
        loss_t2i = F.cross_entropy(logits.t(), labels)              # 文→图: 对称
